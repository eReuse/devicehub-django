...
configure_smtp() {
        ...
        2 | yes | y)
                ...
                export DEVICEHUB_EMAIL_ENABLED_REQUEST='true'
                printf '\n✓ SMTP configured. Emails will be enabled.\n'
                ;;
        *)
                # Set defaults for disabled email
                export DEVICEHUB_EMAIL_ENABLED_REQUEST='false'
        ...
        add_env_var DEVICEHUB_EMAIL_ENABLED_REQUEST
...
docker_wizard__finalize() {
	...
	if [ -z "${RPROXY_TEMPLATE_NAME_REQUEST:-}" ]; then
		export RPROXY_TEMPLATE_NAME_REQUEST='app.template'
	fi
	export RPROXY_TEMPLATE_REQUEST="/etc/nginx/conf.d/${RPROXY_TEMPLATE_NAME_REQUEST}"
	add_env_var RPROXY_TEMPLATE_NAME_REQUEST
	add_env_var RPROXY_TEMPLATE_REQUEST
        ...
        export TIME_ZONE_REQUEST="$(cat /etc/timezone)"
...
docker_wizard__prod_rproxy_external_tls() {
		...
		export RPROXY_TEMPLATE_NAME_REQUEST='app.http.template'
}

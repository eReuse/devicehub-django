...
test('Search', async ({ page }) => {
    ...
    //search by all
    await page.getByPlaceholder('Search by all, or append :').fill('7');
    await page.getByRole('button', { name: '' }).click();

    //search by id
    await page.getByPlaceholder('Search by all, or append :').fill('7:shortid');
    await page.getByRole('button', { name: '' }).click();

    //search by cpu
    await page.getByPlaceholder('Search by all, or append :').fill('7:cpu');
    await page.getByRole('button', { name: '' }).click();

    //search by current_state
    await page.getByPlaceholder('Search by all, or append :').fill('inbox:current_state');
    await page.getByRole('button', { name: '' }).click();

    //search by manufacturer
    await page.getByPlaceholder('Search by all, or append :').fill('dell:manufacturer');
    await page.getByRole('button', { name: '' }).click();

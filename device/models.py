...
class Device:
    ...
    def get_all_sql(cls, institution, offset=0, limit=None):
        evs = q_sql.queryset_SQL(institution.id, offset=offset, limit=limit)
        count = q_sql.queryset_SQL_count(institution.id)
        devices = [cls(id=x[0], owner=institution) for x in evs]
        return devices, count

    @classmethod
    def get_unassigned_sql(cls, institution, offset=0, limit=20):
        evs = q_sql.queryset_SQL_unassigned(institution.id, offset=offset, limit=limit)
        count = q_sql.queryset_SQL_unassigned_count(institution.id)
        devices = [cls(id=x[0], owner=institution) for x in evs]
        return devices, count
